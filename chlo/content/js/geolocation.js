...
if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
  if(navigator.geolocation){
    navigator.geolocation.getCurrentPosition(fetchZomatoByGps);
}
}
...
let findEstablishment = (id)=>
fetch(`https://developers.zomato.com/api/v2.1/establishments?city_id=${id}`,{ method: 'GET', headers: new Headers({'user-key': '4319603cbb48b9c4fb5a3211714b89d1'})})
.then(res => res.json()).then(data => showPlacesType(data.establishments))
...
document.querySelector('.planDate').addEventListener('click',()=>showPlaces(inp.nearby_restaurants));
